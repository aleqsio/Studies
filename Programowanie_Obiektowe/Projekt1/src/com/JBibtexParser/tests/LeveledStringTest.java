...
class LeveledStringTest {
    ...
    void getSubstringsOfLevelShouldWorkCorrectlyWithBrackets() throws ParseErrorException {
        ...
        assertArrayEquals(new String[]{"test"},s1.getSubstringsOfLevel(2, (x, y) -> -y + x).stream().map(p->p.getEntry()).toArray());

       }
    @org.junit.jupiter.api.Test
    void getSubstringsOfLevelShouldWorkCorrectlyWithQuotes() throws ParseErrorException {
        LeveledString s1 = new LeveledString("\"test\"");
        assertArrayEquals(new String[]{"test"},s1.getSubstringsOfLevel(1, (x, y) -> x-y).stream().map(p->p.getEntry()).toArray());
    ...
    void splitOnLevel() throws ParseErrorException {
        ...
        assertArrayEquals(new String[]{"a","b","{c,d}"},s1.splitOnLevel(',',0).stream().map(p->p.getEntry()).toArray());
